  ...
  return (
    <>
      ...
      <main className="space-y-6 p-8">
        <form onSubmit={saveSettings} className="grid grid-cols-2 gap-4 rounded-lg border border-border bg-card p-5">
          <Input name="bundlePrice" label="Bundle Price" type="number" defaultValue={settings.data?.bundlePrice || 4999} />
          <label className="flex items-center gap-2"><input name="maintenanceMode" type="checkbox" defaultChecked={settings.data?.maintenanceMode} /> Maintenance Mode</label>
          ...
          <label className="col-span-2 space-y-2 text-sm"><span>Company Description</span><textarea name="description" className="min-h-24 w-full rounded-md p-3" defaultValue={company.description} /></label>
          <label className="space-y-2 text-sm"><span>Mission</span><textarea name="mission" className="min-h-24 w-full rounded-md p-3" defaultValue={company.mission} /></label>
          <label className="space-y-2 text-sm"><span>Vision</span><textarea name="vision" className="min-h-24 w-full rounded-md p-3" defaultValue={company.vision} /></label>
          <Button className="col-span-2">Save Settings</Button>
        </form>
        <section className="rounded-lg border border-border bg-card p-5">
          <h2 className="mb-4 text-xl font-bold">Change Password</h2>
          <div className="flex gap-3"><Input type="password" placeholder="New password" value={password} onChange={(e) => setPassword(e.target.value)} /><Button onClick={async () => { await api.put('/admin/password', { password }); setPassword(''); toast.success('Password changed'); }}>Update</Button></div>
        </section>
        <section className="rounded-lg border border-border bg-card p-5">
          <h2 className="mb-4 text-xl font-bold">App Banners</h2>
          <form onSubmit={saveBanner} className="mb-4 grid grid-cols-4 gap-3"><Input name="title" label="Title" required /><Input name="subtitle" label="Subtitle" /><Input name="imageUrl" label="Image URL" /><Input name="order" label="Order" type="number" /><label className="col-span-4 rounded-md border border-dashed border-border bg-secondary p-4 text-sm"><span className="block font-semibold text-gold">Upload Banner Image</span><input className="mt-2" type="file" accept="image/*" onChange={(e) => setBannerImage(e.target.files?.[0] || null)} /><span className="mt-2 block text-muted">{bannerImage ? bannerImage.name : 'Optional when Image URL is filled.'}</span></label><Button className="col-span-4">Add Banner</Button></form>
